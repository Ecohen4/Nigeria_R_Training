...
There are many different data formats in wide use, each with it's own strengths and weaknesses. A few of the most common for use in R include:
* .csv
* .xlsx
* .txt
* .ncdf

.csv is the prefered data format for importing into R. Although there are functions in R to read other data formats (a few examples, below), we recommend that you convert to csv prior to loading. Motivation for using csv formats can be found [here](http://dataprotocols.org/simple-data-format/#why-csv).

For a more complete description of various data formats, refer [here] (http://cran.r-project.org/doc/manuals/r-release/R-data.html#Importing-from-other-statistical-systems).
